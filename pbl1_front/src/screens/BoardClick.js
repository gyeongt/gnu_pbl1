  ...
  useEffect(() => {
    const fetchBoardDetail = async () => {
      try {
        ...
        setUserSeq(await AsyncStorage.getItem('userSeq'));
...
// comments 배열을 처리하여 delete_Flag가 1인 댓글은 '삭제된 댓글입니다'로, 0인 댓글은 원래 댓글을 그대로 유지
  ...
  const sendComment = async () => {
    ...
    try {
      ...
      setComments([
        ...comments,
        { commentSeq: comments.length + 1, nickName: comments.nickName, commentText: newComment },
      ]);
      setNewComment('');
    } catch (error) {
...
<View style={styles.commentInputContainer}>
  ...
  onPress={() => {
    ...
    // 답글을 다는 상태에서만 상태를 반전시킴
    if (isReplying) {
      // 답글을 다는 경우에는 상태를 다시 초기화하지 않음
      setIsReplying(false);
    } 
  }}
>
